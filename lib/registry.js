...
Registry.prototype.removeSchema = function(name) {
  if (!this.schemas[name]) {
    return new Error('Unknown schema "' + name + '"');
  }
...
Registry.prototype.addSchema = function(definition) {
  if (typeof definition.id !== 'string' || definition.id.length < 2) {
    return new TypeError('Invalid schema id');
  }

  try {
    this.schemas[definition.id] = new Schema(this, definition);
  } catch (err) {
    return err;
  }
};
